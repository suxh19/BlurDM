...
def save_visualizations(out_dir: str, vis_images: list[dict], epoch: int) -> None:
    """保存可视化图像
    ...
    for idx, img_dict in enumerate(vis_images):
        gt = shift_to_unit_np(img_dict["gt"].squeeze().numpy())
        degraded = shift_to_unit_np(img_dict["degraded"].squeeze().numpy())
        pred = shift_to_unit_np(img_dict["pred"].squeeze().numpy())
        ...
        ax2.set_title("Degraded", fontsize=12, fontweight="bold")
        ax2.axis("off")
