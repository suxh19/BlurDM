...
def build_MIMOUnet_net(model_name: str, num_res: int = 20, in_channels: int = 1):
    """Build MIMO-UNet model.
    
    Args:
        model_name: Model name, should be "MIMOUNetBlurDM"
        num_res: Number of residual blocks (default: 20)
        in_channels: Number of input channels (default: 1 for CT)
    
    Returns:
        MIMOUNetPlusPrior model
    """
    ...
    if model_name == "MIMOUNetBlurDM":
        return MIMOUNetPlusPrior(num_res=num_res, in_channels=in_channels)
    raise ModelError('Wrong Model!\\nYou should choose MIMOUNetBlurDM.')
