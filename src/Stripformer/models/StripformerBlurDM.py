...
class Embeddings_output(nn.Module):
    ...
    def forward(self, x, residual_1, residual_2, prior):


        hx = self.de_layer3_1(x)

        hx = self.de_layer2_2(torch.cat((hx, residual_2), dim = 1))
        hx = self.de_block_1(hx, prior)
        hx = self.de_block_2(hx, prior)
        hx = self.de_block_3(hx, prior)
        hx = self.de_block_4(hx, prior)
        hx = self.de_block_5(hx, prior)
        hx = self.de_block_6(hx, prior)
...
class Intra_SA(nn.Module):
    def __init__(self, dim, head_num):
        ...
        self.PEG = PEG(dim)
        self.kv = nn.Linear(256, 2 * dim, bias=False)
...
class StripformerPrior(nn.Module):
    def __init__(self):
        super(StripformerPrior, self).__init__()
    ...
    def forward(self, x, prior):
        ...
        hx = self.Trans_block_1(hx, prior)
        hx = self.Trans_block_2(hx, prior)
        hx = self.Trans_block_3(hx, prior)
        hx = self.Trans_block_4(hx, prior)
        hx = self.Trans_block_5(hx, prior)
        hx = self.Trans_block_6(hx, prior)
        hx = self.Trans_block_7(hx, prior)
        hx = self.Trans_block_8(hx, prior)
        hx = self.Trans_block_9(hx, prior)
        hx = self.Trans_block_10(hx, prior)
        hx = self.Trans_block_11(hx, prior)
        hx = self.Trans_block_12(hx, prior)
        hx = self.decoder(hx, residual_1, residual_2, prior)

        return hx + x

def get_generator(model_config):
    generator_name = model_config
    if generator_name == 'StripformerPrior':
        model_g = StripformerPrior()
    else:
